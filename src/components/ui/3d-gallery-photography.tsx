...
export type ImageItem = string | { src: string; alt?: string; videoId?: string };
...
interface InfiniteGalleryProps {
    ...
    onItemClick?: (image: ImageItem) => void;
}
...
function GalleryScene({
    ...
    useFrame((state, delta) => {
        // Apply auto-play
        if (autoPlay) {
            setScrollVelocity((prev) => prev + 2.5 * delta);
        }
            ...
            {planesData.current.map((plane, i) => {
                ...
                return (
                    <ImagePlane
                        ...
                        onClick={() => {
                            if (onItemClick) {
                                onItemClick(normalizedImages[plane.imageIndex]);
                            }
                        }}
                    />
                );
...
function FallbackGallery({ images, onItemClick }: { images: ImageItem[], onItemClick?: (img: ImageItem) => void }) {
    const normalizedImages = useMemo(
        () =>
            images.map((img) =>
                typeof img === 'string' ? { src: img, alt: '' } : img
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-[#050505] p-4 text-white">
            <p className="text-white/60 mb-4 font-serif italic">
                Alternative View (WebGL disabled):
            </p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
                {normalizedImages.map((img, i) => (
                    <img
                        ...
                        onClick={() => onItemClick && onItemClick(img)}
                    />
                ))}
            </div>
        </div>
    ...
    if (!webglSupported) {
        return (
            <div className={className} style={style}>
                <FallbackGallery images={images} onItemClick={onItemClick} />
            </div>
        );
    }
