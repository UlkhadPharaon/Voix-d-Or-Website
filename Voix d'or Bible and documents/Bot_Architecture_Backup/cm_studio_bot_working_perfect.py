...
REPLIED_COMMENTS_FILE = os.path.join(BASE_DIR, "replied_comments.json")
...
def publish_to_facebook(image_path_or_url, message_text):
...
- PROMPT IMAGE : Style Cinématographique, 8K, éclairage dramatique, texture dorée métallique. Inclure motifs culturels Burkinabè modernisés."""
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": system_prompt + "\nIMPORTANT: Tu dois renvoyer STRICTEMENT un objet JSON valide, sans aucun texte avant ni après."},
    ...
    try:
        ...
        if "choices" in res_json:
            ...
            print(f"[DEBUG] Extracted content to parse: {content}")
            try:
                data = json.loads(content)
                # Fallback on "textOutput" just in case, but primary is "facebookCaption"
                fb_cap = data.get("facebookCaption") or data.get("textOutput")
                return fb_cap, data.get("imagePrompt")
            except Exception as e:
...
def generate_optimized_prompt(image_path, user_instruction):
    ...
    system_prompt = (
        ...
        "REGLE ABSOLUE 1 : N'inclus JAMAIS aucun humain, aucun visage, aucune silhouette dans le prompt quand aucune image de référence n'est donnée. Reste concentré sur l'ambiance, les objets, l'art, l'architecture ou le matériel. "
        "REGLE DE LAYOUT : NE RÉDUIS JAMAIS la taille de l'image originale. La photo doit occuper 100% de l'espace. "
        "REGLE DU LOGO : Le logo et la bannière seront ajoutés par moi-même après génération. NE DEMANDE JAMAIS À L'IA D'AJOUTER LE LOGO OU DU TEXTE. "
...
def apply_branding(image_url):
    print("[Branding] Application du vrai logo et de la bannière via PIL...")
    try:
        ...
        # Redimensionner l'image de base pour un standard carré si nécessaire
        img = img.resize((1024, 1024), Image.Resampling.LANCZOS)
        
        # Appliquer le logo en haut à gauche
        if os.path.exists(LOGO_PATH):
            logo = Image.open(LOGO_PATH).convert("RGBA")
            # Redimensionner le logo (ex: 200px de large)
            l_width = 250
            l_ratio = l_width / logo.width
            l_height = int(logo.height * l_ratio)
            logo = logo.resize((l_width, l_height), Image.Resampling.LANCZOS)
            img.paste(logo, (30, 30), logo)
            
        # Appliquer la bannière en bas
        if os.path.exists(BANNER_PATH):
            banner = Image.open(BANNER_PATH).convert("RGBA")
            # Redimensionner la bannière pour la largeur de l'image
            b_width = 1024
            b_ratio = b_width / banner.width
            b_height = int(banner.height * b_ratio)
            banner = banner.resize((b_width, b_height), Image.Resampling.LANCZOS)
            img.paste(banner, (0, 1024 - b_height), banner)
            
        # Sauvegarder
        output_path = os.path.join(BASE_DIR, "assets", "branded_output.png")
        img.convert("RGB").save(output_path, format="PNG")
        print("[Branding] Branding appliqué avec succès.")
        return output_path
    except Exception as e:
        print(f"[Branding Error] {e}")
        return image_url # Fallback sur l'URL originale
...
def background_processing_task(inst, m_path):
    ...
    # On ne crée PLUS de fond uni pour forcer le mode Image-to-Image ! 
    # Cela casse la règle absolue de "No human" quand aucune référence n'est donnée.
    
    final_img_url = process_with_fal(img_prompt, m_path)
    if final_img_url:
        branded_img_path = apply_branding(final_img_url)
        fb_res = publish_to_facebook(branded_img_path, fb_cap)
        if fb_res.get("id"):
            send_whatsapp_message(ULKHAD_PHONE, f"✅ Publication réussie sur la page !\n\nLégende générée :\n{fb_cap}\n\nLien de l'image (sans overlay) : {final_img_url}")
        else:
